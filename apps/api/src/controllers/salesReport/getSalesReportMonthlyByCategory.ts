...
const GetSalesReportMonthlyByCategory = async (req: Request, res: Response) => {
  try {
    ...
    if (getOrderData.length === 0) {
      return res.status(400).json({
        code: 400,
        message: 'no report within date range or category',
      });
    }
...
export default GetSalesReportMonthlyByCategory;
